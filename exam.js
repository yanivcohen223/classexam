...
const data = {
    tv: [
        {
        image:'https://www.google.com/search?q=TCL+24+Inch+HD+Ready+24D3100+-+black&sxsrf=AJOqlzX8DCxkWEh603iWVXZ_zDhasxdCVg:1674747027234&source=lnms&tbm=isch&sa=X&ved=2ahUKEwi1lZbtxuX8AhWC26QKHbwuCX8Q_AUoAnoECAEQBA&biw=1490&bih=714&dpr=1.25',
...
for(let i=0; i<data.tv.length; i++){
    $("#tv_tab").append(`
        <tr id="tr${i+1}">
        <td>${i+1}</td>
        <td><a href="${data.tv[i].image}"><img src="/images/tv_image.png" alt=""></a></td>
        ...
        <td><a href="${data.tv[i].store_link}"><img src="${data.tv[i].store_pic}" alt=""></a>    </td>
        <td>${data.tv[i].rating}/5</td>
        <td><button type="button" id="deleteBtn${i+1}" class="btn btn-danger">delete</button></td>
        <td><button type="button" id="buyBtn${i+1}" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#staticBackdrop${i+1}">buy</button></td>
        </tr>`)
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="staticBackdropLabel">${data.tv[i].tv_model}</h5>
...
function price_avg(arr_objects){
    ...
    for(let i=0; i<arr_objects.object.length; i++) {
        avg.price_avg = avg.price_avg+arr_objects.object[i].price
        avg.items_num++
    }
...
function best_option(arr_obj) {
    arr_obj.obj.sort((a,b) => b.rating > a.rating ? -1: 1).sort((a,b) => b.price > a.price? -1 : 1) //sort by rating

    return arr_obj.obj[0].id
}